...
| **SQL Server** | Database — managed CDC | **GA** (per Databricks product page) | **Yes** in the standard two-pipeline architecture; **No** if you use the newer **integrated CDC** single-pipeline architecture | `lakeflow-connect/sql-server-overview` |
...
For these nine, there is no scriptable path to the initial token. Full stop. Any automation you build must stop and hand off to a human. (Source: managed-connector overview, "Create connections programmatically".)
...
2. **SaaS with API-key/token auth (NetSuite, ServiceNow-via-ROPC, Google Analytics-via-service-account, Salesforce-via-mTLS)** → automate the Databricks side; generate a precise, checklist-style handoff for the source-side admin.
3. **The nine browser-OAuth-only connectors, plus Workday** → do not attempt automation of the connection step. Generate the runbook, then block on a human.
